...
def get_valuation_data_syndicate(ticker):
    """
    Syndicate fetching for Analyst Targets (Finviz -> MarketWatch)
    """
    data = {"target": 0.0, "source": "N/A"}
    ...
    try:
        ...
        # Look for Target Price in the huge snapshot table
        table = soup.find('table', {'class': 'snapshot-table2'})
        if table:
            for row in table.find_all('tr'):
                cells = row.find_all('td')
                for i, cell in enumerate(cells):
                    if "Target Price" in cell.text:
                        val = cells[i+1].text.strip()
                        data["target"] = float(val) if val != '-' else 0.0
                        data["source"] = "Finviz"
                        if data["target"] > 0: return data
    except:
        pass
